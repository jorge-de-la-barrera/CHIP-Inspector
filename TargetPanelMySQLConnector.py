...
class TargetPanelMySQLConnector(TargetPanelDBConnector):
    ...
    def add_gene(self, panel_name, symbol, transcript):
        query = f"INSERT INTO CHIP_INSPECTOR.target_panel (panel_name, gene, feature) " \
                f"VALUES ('{panel_name}', '{symbol}', '{transcript}')"
        cursor = self.connection.cursor()
        cursor.execute(query)

    def erase(self, panel_name):
        query = f"DELETE FROM CHIP_INSPECTOR.target_panel WHERE panel_name = '{panel_name}'"
        cursor = self.connection.cursor()
        cursor.execute(query)
        self.connection.commit()

    def get(self, panel_name):
        query = f"SELECT GENE, FEATURE FROM CHIP_INSPECTOR.target_panel WHERE panel_name = '{panel_name}'"
        cursor = self.connection.cursor()
        cursor.execute(query)
