...
'''
    Lee el fichero VCF con todas las anotaciones y genera la salida de aquellas mutaciones, seleccionadas   
'''
...
def parse(config, annotated_vcf, tsv_outfile, translation_filename, bypass_vaf_filter=False):
    ...
    #
    def get_longitudinal(filename):
        """ Crea una tabla que asocia SEQN con Sample_id para las muestras """

        res = dict()
        with open(filename, 'r') as data:
            for file_line in data:
                seqn, vis1, vis3 = file_line.strip().split()
                res[vis1] = (seqn, "visita1")
                res[vis3] = (seqn, "visita3")
        return res

    def is_in_vaf_threshold_exception_gene():
        """
            Listado de genes para los que no se aplica el threhold de la vaf
    ...
    def get_sample_data(all_alleles, sample_names, vcf_format, vcf_samples):
        """
            Devuelve un dict donde la key es la muestra y el value es otro dict con los datos de cada muestra (GT, DP, etc..)
        """

        def get_sample_info(vcf_format, sdata):
            ''' Devuelve un dict con los campos FORMAT de cada muestra'''
    ...
    def OLD_get_sample_name(filename, samples_vcf):
        """ Get the id of the sample from thw CVF file and convert it to the propper id
            VERSION OBSOLETA
        """
        ...
        def create_translation_table():
            ...
            with open(filename, 'r') as data:
                for file_line in data:
                    seqn, id, id_genomica = file_line.strip().split("\t")
                    res[id_genomica] = id
            return res

        translation_table = create_translation_table()
        # return [translation_table[vcf_id] for vcf_id in samples_vcf]
        res = list()
        for vcf_id in samples_vcf:
            # A veces tiene dos codigos separados por comas. Nos quedamos con el primero y quitamos '_Rep' si existe
            sample_id = vcf_id.split(',')[0].replace('_Rep', '')

            if sample_id.startswith("Sample_PESAV"):
                item = translation_table[sample_id[7:]]
            elif sample_id.startswith("PESAV"):
                item = translation_table[sample_id]
            elif sample_id.startswith("Sample_AA") or sample_id.startswith("Sample_1"):
                item = sample_id[7:]
            elif sample_id.startswith("AA") or sample_id.startswith("1"):
                item = sample_id
            else:
                print("Error en la conversion de nombres de muestra")
                exit(1)
    ...
    def OLD_get_annotation():
        """
            Return the principal transcript annottaion and the 'HGVSp' of the other deletereous annotations

            As 'principal' transcript we define: if belongs to the list provided by Mirian or one in the
            most deletereous
        ...
        def get_deletereous(anotaciones):
            def test(a):
                return has_impact(a) and a.get('SYMBOL') not in genes_to_exclude()

            return list(filter(test, anotaciones))

        def is_cannonical(a):
            feature_prefix = a.get('Feature').split('.')[0]
            return a if feature_prefix in canonical_transcripts else False
        ...
        if anotaciones == 'no_annotated':
            ...
            deletereous = get_deletereous(anotaciones)
            principal = [anot for anot in deletereous if is_cannonical(anot)]
            # Si no hay anotado un transcrito principal, sacamos el primer deletereo que haya
            if not principal and deletereous:
                principal = [deletereous[0]]
            other = [anot.get('HGVSp') for anot in deletereous if anot.get('HGVSp', None)]

            return principal[0] if principal else None, other if other else None
    ...
    def get_samples_with_mutation(samples):
        """
            Returns a list of tuples with the sample_id and its vaf of all samples with the mutation
        """
    ...
    def get_samples_with_vaf_like_germinal(samples):
        """
            :return: ID list of samples with a vaf compatible with germinal variant
    ...
    def heuristic_filter(cosmic_match):
        ...
        def do_filter():
            """
                Mutacion con VAF media < 2% (HEURISTIC_VAF_THRESHOLD ) y que esta en mas del 2%
                (HEURISTIC_FRACTION_SAMPLES) de la cohorte.
            """
    ...
    def create_printing_item():
        ...
        def predictor_splitting(prediction_chunk):
            """
                Estre la descripcion y el score de la prediccion (p.e SIFT y PolyPhen)
            """
            ...
            ''''
            Intentar resescribirlo como regex
            m = re.search('([a-zA-Z_])\((.+?)\)', prediction_chunk)
            if m:
                score =  m.group(1)
                k=1
            '''
    ...
    try:
        #
        # Establece los valores de referencia a usar
        # Threshold para emitir una variante putativamente somática
        VAF_THRESHOLD = config.getfloat('PARSING', 'VAF_THRESHOLD')
        # Maxima MAF en la poblacion
        MAX_MAF_THRESHOLD = config.getfloat('PARSING', 'MAX_MAF_THRESHOLD')
        #MAX_MAF_THRESHOLD = float(config.get('PARSING', 'MAX_MAF_THRESHOLD'))
        # Max de muestras con una VAF sospechosa
        ...
        with gzip.open(annotated_vcf, 'rt') as vcf_file:
            for line_chunk in vcf_file:
                if line_chunk.startswith('##INFO=<ID=CSQ'):
                    ...
                    # Obtiene la información del locus/var
                    ...
                    # NIVEL locus/var -> anotacion
                    # Obtiene la info de las anotaciones
                    # anotacion, other_annotations = get_annotation()
                    anotacion = get_annotation()

                    if anotacion and is_deleterious(anotacion):
                        # NIVEL locus/var -> muestra
                        # Obiene la informacion de cada muestra
                        samples_gt = get_sample_data([vcf_ref, *vcf_alt.split(',')], sample_names, vcf_format,
                                                     vcf_samples)

                        # Muestras con la mutacion (cualquier VAF)
                        samples_with_mutation = get_samples_with_mutation(samples_gt)
                        # Muestras con una mutacion compatible con germinal
                        ...
                        if is_whitelisted or seems_somatic():
                            # Bucle principal para emitir las mutacion/muestra de ese locus
