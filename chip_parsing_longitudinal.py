...
'''
    Lee el fichero VCF con todas las anotaciones y genera la salida de aquellas mutaciones, seleccionadas para 
    un estudio longitudinal
        
        1) Con impacto funcional (p.e: HIGH o MODERATE)
        2) Que no son germinales. Una variante se considera germinal si:
            - se detecta en > MAX_NUM_SAMPLES_WITH_VAR muestras 
            - aparece en ALGUNA muestra con una VAF entre 0.45 y 0.55 o con VAF  0.85
            - No está en la población con MAF > MAX_MAF_THRESHOLD
            - Está en un máximo de MAX_NUM_SAMPLES_GERMINAL_VAF con una VAF ger
            minal
        3) Con una VAF > VAF_THRESHOLD
...
def parse(config, annotated_vcf, prefix_outfile, translation_filename, bypass_vaf_filter=False):
    def get_longitudinal_pairs(filename):
        """ Crea dos dict que mapean de sample_id -> seqn y de seqn -> todas sus sample_id"""

        # """ Crea una tabla que asocia SEQN (primera col) con Sample_id para las visitas (resto de cols) """

        # res = list()
        # with open(filename, 'r') as data:
        #     for file_line in data:
        #         res.append(tuple(file_line.strip().split("\t")))
    ...
    def get_whitelist_ncl():
        ...
        return [record[0].decode('ascii') for record in chip_cfg.exec_query(db, query)]
    ...
    def get_samples_with_mutation(samples):
        ...
        # Implementar el lanzamiento de una excepcion si no hay campo AF (o no es numerico)
    ...
    bypass_cosmic = False  # argumento para saltarse la comprobacion de COSMIC
            ...
            if line_chunk.startswith('##INFO=<ID=CSQ'):
                ...
                if anotacion:
                    ...
                    if is_whitelisted or seems_somatic():
                        # Bucle principal para emitir las mutaciones de ese locus para aquellas muestras
                        # del VCF tenemos identificadas como longitudinales
                        for seqn in set([seqn for sampleid, vaf in samples_with_mutation
                            ...
                            # Solo emite la mutacion si esta en ambas visitas
                            if ((tmp_vis1 := samples_gt.get(vis1, {}).get(variante, False))
                                ...
                                # Si alguno de los timepoints tiene una VAF válida y no se considera germinal
                                if not is_vaf_germ(vaf_vis1, nocall_as_germ=True) or \
                                        not is_vaf_germ(vaf_vis3, nocall_as_germ=True):
                                    # Si esta activado, se genera un fichero dump con todas las mutaciones (no-germinales)
                                    # detectadas
                                    if longitudinal_distribution_dump:
                                    ...
                                    if not bypass_vaf_filter:
                                        # Asumimos (por el codigo anterior) que hay, al menos, una VAF valida
